...
def parse_args():
    ...
    p.add_argument(
        ...
        action="append",
        default=[],
        dest="skill_artifacts",
        help=(
            "Path to an evolved H5 skill artifact JSON. Can be passed multiple "
            "times. Used only when --h5 is enabled."
        ),
    )
    p.add_argument(
        "--user-prompt-override",
        default=None,
        dest="user_prompt_override",
        help=(
            "Path to a JSON file mapping task_id to revised user_simulator "
            "instructions. Format: {\"overrides\": {task_id: text, ...}}. "
            "Tasks whose ids are absent are evaluated with the original scenario."
        ),
    )
    p.add_argument(
        "--retrieve-policy",
    ...
    p.add_argument(
        "--agent-api-key-env",
        default=os.getenv("AGENT_API_KEY_ENV", "AGENT_API_KEY"),
        help=(
            "Environment variable to use for the agent LLM API key. Default: "
            "AGENT_API_KEY_ENV if set; otherwise AGENT_API_KEY."
        ),
    )
    p.add_argument(
        "--agent-max-tokens",
    ...
    p.add_argument(
        "--agent-disable-thinking",
        action="store_true",
        help=(
            "Request non-thinking mode for agent LLM providers that support it. "
...
def _is_invalid_tool_failure(sim) -> bool:
    """Treat agent-selected nonexistent tools as task failures, not infra failures."""
    if getattr(sim, "termination_reason", None) != "infrastructure_error":
        return False
    info = getattr(sim, "info", None) or {}
    error = info.get("error") or ""
    return error.startswith("Unknown tool ") and "does not exist" in error


def _invalid_tool_failure_record(sim) -> dict:
    info = getattr(sim, "info", None) or {}
    return {
...
def _apply_user_prompt_overrides(tasks: list[Task], override_path: str) -> list[Task]:
    """Replace the user_scenario.instructions for tasks whose id has an override.

    Expected JSON format: ``{"overrides": {task_id: revised_instructions, ...}}``.
    """
    payload = json.loads(Path(override_path).read_text())
    overrides = payload.get("overrides") or {}
    if not overrides:
        print(f"  [--user-prompt-override] {override_path} has no overrides; pass-through")
        return tasks
    applied = 0
    new_tasks: list[Task] = []
    for task in tasks:
        revised = overrides.get(str(task.id)) or overrides.get(task.id)
        if not revised:
        ...
        if isinstance(instructions, StructuredUserInstructions):
            new_instructions = instructions.model_copy(
                update={"task_instructions": str(revised)}
            )
        else:
            new_instructions = str(revised)
        new_scenario = scenario.model_copy(update={"instructions": new_instructions})
        new_tasks.append(task.model_copy(update={"user_scenario": new_scenario}))
        applied += 1
    print(
        f"  [--user-prompt-override] applied to {applied}/{len(tasks)} tasks "
        f"from {override_path}"
    )
    return new_tasks


def _count_applied_overrides(tasks: list[Task], override_path: str | None) -> int:
    ...
    overrides = payload.get("overrides") or {}
    return sum(
        1 for task in tasks if (str(task.id) in overrides) or (task.id in overrides)
    )
...
def summarise(results, label: str, pass_k: int = 1) -> dict:
    ...
    invalid_tool_failures: list[dict] = []

    for seq, sim in enumerate(results.simulations):
        ri = sim.reward_info
        if ri is None:
            if not _is_invalid_tool_failure(sim):
                continue
            tid = sim.task_id
            r = 0.0
            invalid_tool_failures.append(_invalid_tool_failure_record(sim))
        else:
            tid = sim.task_id
            r = ri.reward
    ...
    return {
        ...
        "invalid_tool_failure_count": len(invalid_tool_failures),
        "invalid_tool_failures": invalid_tool_failures,
    }


def _infrastructure_errors(results) -> list[dict]:
    """Return concise infrastructure errors recorded by the runner."""
    errors = []
    for sim in getattr(results, "simulations", []) or []:
        if getattr(sim, "termination_reason", None) != "infrastructure_error":
            continue
        if _is_invalid_tool_failure(sim):
            continue
        info = getattr(sim, "info", None) or {}
        errors.append(
            {
...
def main():
    ...
    if args.skill_artifacts and not harness_h5:
        raise ValueError("--skill-artifact requires --enabled --h5")
    ...
    agent_api_key_source = args.agent_api_key_env.strip() or "AGENT_API_KEY"
    if agent_api_key_source.startswith(("sk-", "sk_", "sk-or-", "sk-ant-")):
        raise ValueError(
            "--agent-api-key-env must be the name of an environment variable, "
            "not the API key value itself."
        )
    agent_api_key = os.getenv(agent_api_key_source)
    if not agent_api_key:
        raise ValueError(f"{agent_api_key_source} is not set for --agent-api-key-env")
    llm_args_agent["api_key"] = agent_api_key
    if args.agent_max_tokens > 0:
        llm_args_agent["max_tokens"] = args.agent_max_tokens
    if args.agent_disable_thinking:
        if "dashscope.aliyuncs.com" in args.agent_api_base:
            llm_args_agent["extra_body"] = {"enable_thinking": False}
        elif "openrouter.ai" in args.agent_api_base:
            llm_args_agent["extra_body"] = {"reasoning": {"enabled": False}}
        else:
            llm_args_agent["extra_body"] = {
    ...
    if user_api_base:
        ...
        if args.user_disable_thinking:
            if "dashscope.aliyuncs.com" in user_api_base:
                extra_body["enable_thinking"] = False
            elif "openrouter.ai" in user_api_base:
                extra_body["reasoning"] = {"enabled": False}
            else:
    ...
    config_kwargs: dict = dict(
        ...
        harness_skill_artifacts=args.skill_artifacts,
    ...
    print(f"  Agent API base: {args.agent_api_base}")
    print(f"  Agent API key : {agent_api_key_source}")
    print("  Agent temp  : 0.0")
    print(f"  Agent max tokens: {args.agent_max_tokens or 'provider default'}")
    print(f"  Agent disable thinking: {args.agent_disable_thinking}")
    ...
    infra_errors = _infrastructure_errors(results)

    summary_path = save_dir / "harness_summary.json"
    summary["domain"] = args.domain
    summary["agent_api_base"] = args.agent_api_base
    summary["agent_api_key_env"] = agent_api_key_source
    summary["agent_temperature"] = 0.0
    summary["agent_max_tokens"] = args.agent_max_tokens or None
    summary["agent_disable_thinking"] = args.agent_disable_thinking
    ...
    summary["skill_artifacts"] = args.skill_artifacts
    summary["user_prompt_override"] = args.user_prompt_override
    summary["user_prompt_overrides_applied"] = (
        _count_applied_overrides(tasks, args.user_prompt_override)
        if args.user_prompt_override
        else 0
    )
    ...
    if infra_errors:
        ...
        for err in infra_errors[:5]:
            print(
                f"    task {err.get('task_id')} trial {err.get('trial')}: "
                f"{err.get('error_type')}: {err.get('error')}"
            )
        if len(infra_errors) > 5:
            print(f"    ... {len(infra_errors) - 5} more")
        return 2

    if tasks and summary["total_simulations"] == 0:
        print("\n  ERROR: no completed simulations with reward_info were produced")
        return 3
